...
class ReportService:
    ...
    def _build_slide_outline(self, analysis: Dict, provider: str, model_name: str) -> List[Dict]:
        ...
        rag_sources = [
            f"{item.get('title', 'Untitled')} ({item.get('source_name') or 'source'}, score={item.get('score')})"
            for item in (analysis.get("rag_context", {}).get("chunks") or [])[:4]
        ]
        if rag_sources:
            fallback.insert(3, {"title": "RAG 交叉比對", "bullets": rag_sources})
...
"""
    ...
    def _build_markdown_report(
        ...
        if analysis.get("notes"):
            ...
            lines.append("")
        rag_chunks = analysis.get("rag_context", {}).get("chunks") or []
        if rag_chunks:
            lines.append("## RAG cross-check sources")
            for item in rag_chunks:
                line = f"- {item.get('title') or 'Untitled'}"
    ...
    def _build_sources_slide(
        ...
        rag_chunks = analysis.get("rag_context", {}).get("chunks") or []
        for index, chunk in enumerate(rag_chunks[:3], start=1):
            bullets.append(
                f"RAG {index}: {chunk.get('title') or 'Untitled'}"
                + (f" | {chunk.get('url')}" if chunk.get("url") else "")
            )
        bullets.append(
